...
ribo_path = "experiments/yeastall.ribo"
coverage_path = "yeastallcoverage.pkl.gz"
reference_file_path = "yeast_transcriptome.fa.gz"
alias_int = int(2)
...
for exp in coverage_dict.keys():
    ...
    for transcript, coverage in coverage_dict[exp].items():
        if coverage is not None:
            #Designates the 5' and the 3'
            start, stop = cds_range[transcript]
            #print(start)
            cds_sequence = sequence[transcript][start: stop]
            #This gives me the coding sequence, so it is the original transcript minus the 5' and 3'
            #I changed the start codon from ATG to UUU but kept all other ATG's the same
            cds_sequence = "UUU" + cds_sequence[3:]
            #print(cds_sequence)
